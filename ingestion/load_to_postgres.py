...
engine = create_engine("postgresql+psycopg2://postgres:postgres@postgres:5432/weather")
...
for file in os.listdir(DATA_DIR):
    if file.endswith(".csv"):
        ...
        df = df.head(100)  # Optional: only first 100 rows for testing
        ...
        # Parse wind data: '230,1,N,0134,1' -> direction: 230, speed: 13.4
        df['wind_direction'] = df['wnd'].astype(str).str.split(',', expand=True)[0]
        df['wind_speed'] = pd.to_numeric(
            df['wnd'].astype(str).str.split(',', expand=True)[3],
            errors='coerce'
        ) / 10
